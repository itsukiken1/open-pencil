...
export interface PenDocument {
  ...
  /**
   * Prototyping connections (source → target, interaction, navigation).
   * Shape mirrors scene-graph/connection.ts#Connection.
   * Optional and forward-compat: older Pencil versions ignore this field.
   */
  prototyping?: PenConnection[]
}
...
export interface VarContext {
  ...
  /**
   * Maps pen.id (as it appears in the .pen file) to the id generated by
   * SceneGraph.createNode. Connections reference pen ids so we need this
   * lookup to import prototyping data post-parse.
   */
  penToGraphIds?: Map<string, string>
}
...
export function buildVarContext(
  ...
  return {
    ...
    penToGraphIds: new Map<string, string>()
  }
}
